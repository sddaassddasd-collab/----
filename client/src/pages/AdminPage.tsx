...
function progressFromState(state: ClientState): { accuracy: number; completedCount: number } {
...
function normalizeFinishedAt(state: ClientState): number | null {
  return typeof state.finishedAt === "number" ? state.finishedAt : null;
}

function compareFinishedAtAsc(stateA: ClientState, stateB: ClientState): number {
